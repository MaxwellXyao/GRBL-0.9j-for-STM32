...
void TIM3_IRQHandler(void)   //TIM3中断:The Stepper Driver Interrupt
{
	if (TIM_GetITStatus(TIM3, TIM_IT_Update) != RESET)  //检查TIM3更新中断发生与否
	{
		...
		// TCNT0 = st.step_pulse_time; // Reload Timer0 counter
		// TCCR0B = (1<<CS01); // Begin Timer0. Full speed, 1/8 prescaler
		HW_TIM_PortResetInterrupt_ValueConfig(71,st.step_pulse_time); 	//设置定时器重装值
      ...
      // Initialize step segment timing per step and load number of steps to execute.    
#ifdef ADAPTIVE_MULTI_AXIS_STEP_SMOOTHING
        HW_TIM_DriverInterrupt_ValueConfig(8,st.exec_segment->cycles_per_tick>>1);		//使能AMASS算法就使用固定值分频【分频值需要调整】
#else
				// With AMASS is disabled, set timer prescaler for segments with slow step frequencies (< 250Hz).
